...
df <- read.csv("/Users/valantrahome/projects/data_science-r/movie_metadata2.csv", header=TRUE, stringsAsFactors = FALSE)
...
```{r data1}
movies_rating_byCountryyear <- df %>% group_by(country, title_year) %>% summarize(imdb_score=round(mean(imdb_score),2), facebook_likes=round(mean(movie_facebook_likes)), movie_cnt=n()) %>% arrange(desc(title_year),desc(imdb_score))
...
```{r data2}
library(ggplot2)
ggplot(movies_country_title,aes( x=  country , y=title_year, size = movies)) + geom_point()
```